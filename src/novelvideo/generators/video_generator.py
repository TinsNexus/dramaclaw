        ...
        async with aiohttp.ClientSession(timeout=self._client_timeout()) as session:
            async with session.post(
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"DramaHubAPI submit returned invalid JSON: {text}"
                    ) from exc
        ...
        async with aiohttp.ClientSession(timeout=self._client_timeout()) as session:
            async with session.get(url, headers=headers or self.headers) as resp:
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"DramaHubAPI task query returned invalid JSON: {text}"
                    ) from exc
        ...
        if url.startswith("data:"):
            ...
            async with aiohttp.ClientSession(timeout=self._client_timeout()) as session:
                async with session.get(url) as resp:
                    if resp.status != 200:
                        raise RuntimeError(
                            f"DramaHubAPI result download failed: HTTP {resp.status}"
                        )
            ...
            if not task_id:
                ...
                return VideoGenResult(
                    ...
                    error=(
                        ...
                        if organization_request
                        else f"No task_id in DramaHubAPI response: {submitted}"
                    ),
                )
            if organization_request:
            ...
            for poll_count in range(max_polls):
                ...
                if status in {"completed", "succeeded", "success", "done"}:
                    ...
                    if not video_url:
                        safe_missing_result_error = (
                            "EGRESS_OPERATION_UNKNOWN"
                            if organization_request
                            else "No video url in DramaHubAPI result"
                        )
                        if organization_request:
                            await self._mark_operation_unknown(
                                operation_port,
                        ...
                        return VideoGenResult(
                            ...
                            error=(
                                safe_missing_result_error
                                if organization_request
                                else f"No video url in DramaHubAPI result: {task}"
                            ),
                            task_id=task_id,
                        )
                    if organization_request:
                ...
                if status in {
                    ...
                    error = (
                        ...
                        or "DramaHubAPI video task failed"
                    )
                    safe_task_error = (
                        "EGRESS_OPERATION_UNKNOWN"
                        if organization_request
