...
The current recommended template includes OpenRouter, VolcEngine, fal.ai, and DoubaoAudio. The recommended speech mapping keeps `index-tts-2` as DramaHub's model ID while routing it through the DoubaoAudio channel to the upstream `seed-audio-1.0` model.
