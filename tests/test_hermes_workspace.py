...
def test_fresh_create_layout(isolated_workspace, repo_skills, repo_plugins):
    ...
    assert "虾导在 DramaHub 会话中面向用户自称“虾导”" in memory
    assert "我是虾导，DramaHub 的小说转视频创作助手。" not in memory
...
def test_legacy_identity_context_is_migrated(isolated_workspace, repo_skills, repo_plugins):
    ...
    assert "我是虾导，DramaHub 的小说转视频创作助手。" not in memory
    assert "DramaHub 管理的虾导会话" in memory
    assert "DramaHub 管理的 Hermes 会话" not in memory
