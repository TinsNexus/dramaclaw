            ...
            {data.generationError && !isGenerating && (
              <div className="flex items-center gap-2 border-b border-red-500/25 bg-red-500/10 px-3 py-2">
                ...
                <span
                  ...
                  title={data.generationError.startsWith('node.scriptNode') ? t(data.generationError) : data.generationError}
                >
  ...
  const handleFileChange = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setUploadError(error instanceof Error ? error.message : t('node.scriptNode.errors.uploadFailed'));
      } finally {
        setUploading(false);
      }
    },
    ...
    return (
      <div className="flex flex-col gap-1">
        <button
          ...
          onClick={openPicker}
          disabled={uploading}
          title={t('node.scriptNode.ui.uploadImage')}
        ...
        {editable ? (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center gap-1 rounded bg-black/60 opacity-0 transition-opacity group-hover:pointer-events-auto group-hover:opacity-100">
            <button
              ...
              onClick={openPicker}
              disabled={uploading}
              title={t('node.scriptNode.ui.replaceImage')}
            ...
            <button
              type="button"
              onClick={() => onCommit?.('')}
              title={t('node.scriptNode.ui.deleteImage')}
        ...
        ? createPortal(
            ...
            >
              ...
              <button
                type="button"
                onClick={() => setPreviewOpen(false)}
                title={t('node.scriptNode.ui.closePreview')}
    ...
    >
      ...
      <div className={`px-3 pt-3 ${panelExpanded ? 'flex-1 overflow-hidden' : ''}`}>
        <textarea
          ...
          placeholder={t('node.scriptNode.ui.placeholder')}
          rows={3}
      ...
      <div className="flex shrink-0 items-center justify-end gap-2 px-3 pb-3 pt-1">
        <div className="flex shrink-0 items-center gap-2">
          <IconButton
            title={t('node.scriptNode.ui.translate')}
          ...
          <button
            type="button"
            disabled={submitDisabled}
            title={t('node.scriptNode.ui.generate')}
...
function ScriptReferenceChip({ reference, index }: ScriptReferenceChipProps) {
  ...
  const titleText = reference.displayName?.trim()
    ...
    : t('node.scriptNode.reference.label', { index: index + 1 });
